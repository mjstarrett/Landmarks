...
public enum HideTheTargetOnStart
{
    Off,
    SetInactive,
    SetInvisible,
...
public class NavigationTaskWithHeracles : ExperimentTask 
{
    ...
    public HideTheTargetOnStart hideTargetOnStart;
	...
	{
        if (!Heracles.heraclesIsEmpty)
        {
            Heracles.heraclesCalled = 1;
        }
        ...
        if (hideTargetOnStart != HideTheTargetOnStart.Off)
        {
            if (hideTargetOnStart == HideTheTargetOnStart.SetInactive)
            {
                destinations.currentObject().SetActive(false);
            }
            else if (hideTargetOnStart == HideTheTargetOnStart.SetInvisible)
            {
                destinations.currentObject().GetComponent<MeshRenderer>().enabled = false;
            }
            else if (hideTargetOnStart == HideTheTargetOnStart.SetProbeTrial)
            {
                destinations.currentObject().SetActive(false);
                destinations.currentObject().GetComponent<MeshRenderer>().enabled = false;
            }
	...
	public override bool updateTask () 
	{
        ...
        if (vrEnabled)
        {
            if (hideTargetOnStart != HideTheTargetOnStart.Off && hideTargetOnStart != HideTheTargetOnStart.SetProbeTrial && ((Time.time - startTime > (showTargetAfterSeconds) || vrInput.TouchpadButton.GetStateDown(Valve.VR.SteamVR_Input_Sources.Any))))
            {
                destinations.currentObject().SetActive(true);
            }

            if (hideTargetOnStart == HideTheTargetOnStart.SetProbeTrial && vrInput.TouchpadButton.GetStateDown(Valve.VR.SteamVR_Input_Sources.Any))
            {
        ...
        if (hideTargetOnStart != HideTheTargetOnStart.Off && hideTargetOnStart != HideTheTargetOnStart.SetProbeTrial && ((Time.time - startTime > (showTargetAfterSeconds) || Input.GetButtonDown("Return"))))
        {
            destinations.currentObject().SetActive(true);
        }

        if (hideTargetOnStart == HideTheTargetOnStart.SetProbeTrial && Input.GetButtonDown("Return"))
        {
            //get current location and then log it
