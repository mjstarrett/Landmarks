...
public class ScaledSpawnListLarge : ExperimentTask {
	...
	public override void startTask () {
		...
		if (order ) {
			...
			ScaledSpawnOrderLarge ordered = order.GetComponent("ScaledSpawnOrderLarge") as ScaledSpawnOrderLarge;

			if (ordered) {
				Debug.Log("ordered");
				Debug.Log(ordered.scaledSpawnOrderLarge.Count);

				if (ordered.scaledSpawnOrderLarge.Count > 0) {
					objs = ordered.scaledSpawnOrderLarge.ToArray();
				}
			}
		}
