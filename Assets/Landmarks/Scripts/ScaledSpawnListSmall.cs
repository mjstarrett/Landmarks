...
public class ScaledSpawnListSmall : ExperimentTask {
	...
	public override void startTask () {
		...
		if (order ) {
			...
			ScaledSpawnOrderSmall ordered = order.GetComponent("ScaledSpawnOrderSmall") as ScaledSpawnOrderSmall;

			if (ordered) {
				Debug.Log("ordered");
				Debug.Log(ordered.scaledSpawnOrderSmall.Count);

				if (ordered.scaledSpawnOrderSmall.Count > 0) {
					objs = ordered.scaledSpawnOrderSmall.ToArray();
				}
			}
		}
