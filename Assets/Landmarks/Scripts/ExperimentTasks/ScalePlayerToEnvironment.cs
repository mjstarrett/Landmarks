...
public class ScalePlayerToEnvironment : ExperimentTask
{
    ...
    public override void TASK_START()
    {
        ...
        else if (manager.userInterface == UserInterface.ViveAndVirtualizer)
        {
            ...
            if (isScaled)
            {
                manager.player.GetComponent<CVirtPlayerController>().movementSpeedMultiplier *= (scaleRatio*2);
            }
            else manager.player.GetComponent<CVirtPlayerController>().movementSpeedMultiplier *= (scaleRatio/2);
